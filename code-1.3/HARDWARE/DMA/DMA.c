...
void DMA_TxConfig(DMA_Stream_TypeDef *DMA_Streamx,u8 chx,u32 par,u32 mar,u16 ndtr)
{ 
	DMA_TypeDef *DMAx;
	u8 streamx;
...
//其余参数与上面函数一致
void DMA_RxConfig(DMA_Stream_TypeDef *DMA_Streamx,u8 chx,u32 par,u32 mar,u16 ndtr)
{ 
	DMA_TypeDef *DMAx;
	u8 streamx;
	if((u32)DMA_Streamx>(u32)DMA2)//得到当前stream是属于DMA2还是DMA1
	...
	DMA_Streamx->CR|=0<<6;  //外设到存储器模式，6,7位决定传输方向，默认为0
	DMA_Streamx->CR|=0<<7;	//外设到存储器模式，6,7位决定传输方向，默认为0
	DMA_Streamx->CR|=1<<8; //非循环模式
	DMA_Streamx->CR|=0<<9;		//外设非增量模式
	DMA_Streamx->CR|=1<<10;		//存储器增量模式
